...
try:
    from memorylake import MemoryLakeMemoryTool, MemoryLakeMemoryToolError
except ModuleNotFoundError:  # pragma: no cover - fallback when not installed
    import sys
    from pathlib import Path

    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
    from memorylake import MemoryLakeMemoryTool, MemoryLakeMemoryToolError  # type: ignore
...
def run_chat(
    ...
    """
    Run chat session with remote MemoryLake server.
    ...
    memory_tool = MemoryLakeMemoryTool(
    ...
    print(
        f"🧠 Claude + MemoryLake Remote demo ({mode_str}) (model: {model})\n"
...
def _handle_local_command(
    ...
    try:
        if name == "help":
            ...
            result = memory_tool.execute(view_cmd)
            print(result)
            ...
            result = memory_tool.execute(create_cmd)
            print(result)
            ...
            result = memory_tool.execute(insert_cmd)
            print(result)
            ...
            result = memory_tool.execute(replace_cmd)
            print(result)
            ...
            result = memory_tool.execute(delete_cmd)
            print(result)
            ...
            result = memory_tool.execute(rename_cmd)
            print(result)
...
def _run_exec_command(memory_tool: MemoryLakeMemoryTool, args: List[str]) -> None:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Anthropic memory tool chat demo with remote MemoryLake server.")
    parser.add_argument(
        "--api-key",
